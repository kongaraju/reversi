...
$check = "select count('id') idcnt from reversivisits where id= " . $user_profile[id];
$check_result = mysql_fetch_array(mysql_query($check));

if (!$check_result[idcnt]) {
	$location = $user_profile[location];
	$sql = "INSERT INTO reversivisits (id,name,emailid,location,dat) VALUES ('$user_profile[id]','$user_profile[name]','$user_profile[email]','$location[name]', CURDATE())";
